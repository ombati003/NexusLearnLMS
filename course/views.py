...
@login_required
@lecturer_required
def program_add(request, pk=None):
    if pk:
    ...
    if request.method == "POST":
        ...
        if not program.title or not program.summary:
            messages.error(request, "All fields are required.")
        else:
            program.save()
            messages.success(request, f"Program '{program.title}' saved successfully!")
...
def course_add(request, pk):
    ...
    return render(
        ...
        {"title": "Add Course", "program": program},
    )
...
def course_edit(request, slug):
    ...
    if request.method == "POST":
        ...
        if course.title and course.code:
            course.save()
            messages.success(request, f"{course.title} ({course.code}) has been updated.")
            return redirect("course_detail", slug=course.slug)
        else:
            messages.error(request, "Title and Code cannot be empty.")
...
@method_decorator([login_required, lecturer_required], name="dispatch")
class CourseAllocationFormView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not lecturer_id or not course_ids:
            messages.error(request, "Lecturer and at least one course must be selected.")
        ...
        allocation.courses.set(course_ids)

        messages.success(request, f"Courses allocated to {lecturer.get_full_name()} successfully.")
        return redirect("course_allocation_view")
...
def edit_allocated_course(request, pk):
    ...
    if request.method == "POST":
        ...
        if not lecturer_id or not course_ids:
            ...
            allocation.lecturer = get_object_or_404(User, pk=lecturer_id)
            allocation.save()
            allocation.courses.set(course_ids)
    ...
    lecturers = User.objects.filter(is_lecturer=True)
    courses = Course.objects.all()
    allocation_courses = allocation.courses.values_list("id", flat=True)
    ...
    return render(
        ...
        "course/course_allocation_form.html",
        {
            ...
            "lecturers": lecturers,
            "courses": courses,
            "allocation_courses": allocation_courses,
        },
    )
